...
all_files = os.listdir("/home/caio/Documents/rsync-bucket/obsidian-vault/Kamioka/Experiment/FEAtest-2403/waveform240328/")   # imagine you're one directory above test dir
#print(all_files)  # won't necessarily be sorted
sorted_files = sorted(all_files)
C1 = sorted(sorted_files[80:1203])
C2 = sorted(sorted_files[1283:]) # events go from index 32 to 1203
#drop first 47 of each list
print(len(C1))
print(len(C2))
f1 = {}
f2 = {}
for i in range(0, len(C1)):
    f1[i] = np.genfromtxt('/home/caio/Documents/rsync-bucket/obsidian-vault/Kamioka/Experiment/FEAtest-2403/waveform240328/'+C1[i],
                  dtype= None,delimiter=',',encoding='latin_1', usecols=(0,1)) # column0: time, column1: amplitude
    
for i in range(0, len(C2)):
    f2[i] = np.genfromtxt('/home/caio/Documents/rsync-bucket/obsidian-vault/Kamioka/Experiment/FEAtest-2403/waveform240328/'+C2[i],
                  dtype= None,delimiter=',',encoding='latin_1', usecols=(0,1)) # column0: time, column1: amplitude
...
def individualWaveform(event):
    ...
    plt.subplot(2,1,1)
    plt.title('plot of:'+C1[event-32])
    plt.plot(X1[event-32],Y1[event-32], 'o', markersize=1)
    plt.xlabel('time (ns)')
    plt.ylabel('voltage (mV)')
    plt.subplot(2,1,2)
    plt.title('plot of:'+C2[event-32])
    plt.plot(X2[event-32],Y2[event-32], 'o', markersize=1, color='purple')
    plt.xlabel('time (ns)')
    plt.ylabel('voltage (mV)')
...
event = 32 # must be greater than 32
